...
- 重构 Web 端实例配置导入与导出流程，改用浏览器文件上传和下载通道，避免依赖服务端本地文件选择器及导出目录。
- 新增 Web 实例目录管理对话框，支持目录浏览、新建文件夹、新建文件、重命名、复制、粘贴和删除等常用操作。
- 增加实例目录边界、目录穿越、符号链接和 Windows 非法名称校验，并将删除操作纳入 Web 高风险一次性确认机制。
- 保持桌面端现有文件选择、服务端落盘导出和资源管理器打开目录逻辑不变，实现 Web 与桌面端按运行环境独立分流。
- 完成前端生产构建、Rust 测试、配置元数据一致性检查、格式检查及 Web 端 Playwright 运行验证。
