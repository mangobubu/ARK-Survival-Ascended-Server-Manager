...
- 将右侧 ServerConfig 扩展到 224 个字段，新增离线突袭保护、衰减、驯养上限、经验制作、聊天语音、伤害恢复及 ASA 启动参数等高频配置，并补齐搜索分组与字段提示。
- 为 `GameUserSettings.ini`、`Game.ini` 和 `Engine.ini` 引入按节与键合并机制，保留社区未知配置、注释和重复项，并通过所有权标记支持自定义 INI 安全清空与改名。
- 完善配置默认值、目标元数据、双向导入和 Rust 消费链校验，修正友军伤害拆分、PvE 衰减反向语义、部落联盟、保存间隔及软驯养上限销毁键名。
- 接入 CurseForge 官方 ASA MOD 搜索目录与加密 API Key 配置，支持分页检索、批量选择、版本信息和更新状态展示。
- 修复桌面端 CurseForge MOD 详情页与 API Key 申请页链接无法可靠唤起系统浏览器的问题，统一通过 Tauri opener 打开，并将桌面权限严格限制在 CurseForge HTTPS 域名范围；Web 端继续使用隔离的新标签页。
- 新增 Web 实例目录管理，支持目录浏览、新建文件与文件夹、重命名、复制、粘贴和删除；配置导入导出改用浏览器上传下载通道，桌面端原有流程保持不变。
- 增加实例目录边界、目录穿越、符号链接和 Windows 非法名称校验，并将删除操作纳入 Web 高风险一次性确认机制。
- 加固实例配置保存、应用重启、安装更新和停止流程的并发互斥、取消等待、进程身份核验与进程树退出确认，避免重复操作和误杀进程。
- 补充配置落盘保留、扩展字段往返、自定义配置安全、生命周期并发、进程终止、Web 文件管理和配置导入导出等回归验证。
- 同步项目版本号至 `0.4.3`，确保前端包版本、Tauri 配置、Cargo 包信息、锁文件和发布标签版本一致。
