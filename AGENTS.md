...
- 如果当前应用时web应用或可以用web调试，则需要主动调用Playwright mcp进行测试。
...
### git 推送规则
  - git 提交消息必须使用中文，要详细，且必须遵守以下示例格式（每个功能说明都需要换行处理）：
  ...
  ```
  - 当推送 v* tag时，会触发releases工作流，需要把更新日志加上，最终在releases发布页中，可以看到每个版本的更新日志

  - 当触发releases 工作流时，仅确认工作流正常启动即可，不要持续监控工作流
